...
const DocDataTable = (props) => {
  ...
  const deleteDocProfile = ()=>{
    
    axios.delete(`/api/deleteDoc?email=${deleteEmail}`)
    .then((response)=>{
  ...
  useEffect(() => {
    editEmail &&
      router.push(
        { pathname: "/admin/edit/doctor-profile", query: { email: editEmail } },
    ...
    <div className="w-full p-6  bg-white">
      ...
      <div className="w-full flex justify-between px-3 pb-3">
        ...
        <button style={pageStyle} className="px-3 h-9 py-1 rounded-sm"
         onClick={()=>{
            router.push("/admin/add/doctor-profile")
         }}
        >
            Add 
        </button>
                    ...
                    >
                      ...
                      field === "Email" ? null : (
                        <div className="flex items-cente text-[16px] text-gray-700">
                          {/* // field name  */}
                          {field === "AccountStatus"
                            ? <span className="w-[118px]">Account Status</span>
                            : field === "createdAt"
                            ? <span className="w-28">Member Since</span>
                            :field
                          }
                          ...
                          {field === "Name" ? (
                            <div className="flex items-center ml-3 justify-center">
                              <img
                                ...
                                alt=""
                              />
                              <div className="inline w-32 overflow-hidden">
                                {item[field]}
                              </div>
                            </div>
                          ) : field === "createdAt" ? (
                            <div>{convertDateTime(item[field])}</div>
                          ) : field === "AccountStatus" ? (
                            <SwitchDoc email={item["Email"]} />
                          ) :
                            item[field] 
                          }
